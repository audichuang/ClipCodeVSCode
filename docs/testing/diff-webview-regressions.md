...
Requires an installed Chrome/Chromium/Edge. `CHROME_BIN` wins if set; otherwise the first browser actually present is used — the Chrome and Chromium app bundles then PATH on macOS, the Chrome and Edge install directories under Program Files / Program Files (x86) / LocalAppData then PATH on Windows (neither adds itself to PATH on a stock install), and chromium / chromium-browser / google-chrome / google-chrome-stable / chrome via PATH on Linux. It used to hardcode the bare name `chromium` off macOS, so a box with Google Chrome and no `chromium` could not run this gate at all — and that reads like a skipped gate rather than a missing one. A missing browser is still a failure, never a silent skip. This command builds current Webview assets first, serves only those assets on localhost, launches an isolated temporary headless browser profile and removes it afterward. It does not use the user's browser profile. Profile removal retries (Chrome's children can still be writing into it) and, if it ultimately fails, warns and leaves the temp directory behind rather than changing the exit code — cleanup must never overwrite the real verdict, in either direction.
